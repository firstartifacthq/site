...
| --- | --- | --- | --- |
| First Artifact logo | `public/brand/first-artifact-logo.png` | Brand mark in the fixed navigation and other approved brand placements. Preserve its aspect ratio; do not crop or recolor the source. | When the visible “First Artifact” name sits beside it, use `alt=""` because the image is redundant. If the mark is the only brand label, use `alt="First Artifact"`. |
