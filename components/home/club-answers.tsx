  ...
  return (
    ...
    >
      <div className="page-shell">
        ...
        <ul className="mt-14 border-t border-white/16">
          {answers.map((item) => (
            <li key={item.id} className="border-b border-white/16 py-8 md:py-10">
              <h3 className="max-w-[28ch] text-2xl font-semibold tracking-[-0.02em] md:text-[1.75rem]">
                {item.question}
              </h3>
              <p className="mt-4 max-w-[46rem] text-base leading-relaxed text-white/72 md:text-lg">
                {item.answer}
              </p>
            </li>
          ))}
