...
function Word({ word, index, total, progress }: { word: string; index: number; total: number; progress: MotionValue<number> }) {
  ...
  return <motion.span style={{ opacity }} className="manifesto-word inline-block">{word}&nbsp;</motion.span>;
}
...
export function ManifestoReveal({ text }: { text: string }) {
  ...
  return (
    <p ref={target} className="text-balance max-w-[24ch] text-[clamp(2rem,5.4vw,5rem)] font-medium leading-[0.98] tracking-[-0.035em]">
      {words.map((word, index) => <Word key={`${word}-${index}`} word={word} index={index} total={words.length} progress={scrollYProgress} />)}
    </p>
  );
}
